 ...
 */
const ytsr = module.exports = async(searchString, options, retries = 3) => {
  if (retries === 0) {
  ...
  if (!parsedResponse.json) {
    ...
    return ytsr(searchString, options, retries - 1);
  }
...
ytsr.version = require('../package.json').version;
 ...
 * @returns {Object} - Parsed search result data.
 */
ytsr.continueRequest = async args => {
 ...
 */
ytsr.getFilters = async(searchString, options) => {
