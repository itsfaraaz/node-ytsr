 ...
 */
const main = module.exports = async(searchString, options, retries = 3) => {
  if (retries === 0) {
  ...
  if (!parsedResponse.json) {
    ...
    return main(searchString, options, retries - 1);
  }
 ...
 * @returns {Object} - Parsed search result data.
 */
main.continueRequest = async args => {
...
main.getFilters = async(searchString, options) => {
  ...
  if (!parsedResponse.json) {
    ...
    return main.getFilters(searchString, options);
  }
